...
//! 振る舞う。接触が短いので、フェルトのピアノハンマーで問題になる「接触時間が
//! 弦の 1 周期を超えて基音より上を励振できない」現象は起きにくい。
//!
//! # 力の式 (Hunt-Crossley 型)
//!
//! ```text
...
pub struct HammerParams {
    ...
    /// # なぜ既定で切ってあるか
    ///
    /// ピアノのフェルトのモデルではこれが**必須**になる。フェルトを硬くして
    /// 明るさを稼ぐと接触時間が実機から外れる — 足りないのは接触時間の短さでは
    /// なく**励振波形の鋭さ**で、それを作るのがこの項。
    ///
...
impl HammerParams {
    ...
    /// ピアノフェルトを貼った面。
    ///
    /// 係数はピアノハンマーの中音域の値 (Chaigne & Askenfelt / Stulov 系の
    ...
    pub fn step(&mut self, string_displacement: f64, string_velocity: f64, dt: f64) -> f64 {
        ...
        let force = if compression > 0.0 {
            ...
            if self.state == HammerState::InContact {
                ...
                //
                // 当初は最初の離脱で Released に落としていた (ピアノ流)。
