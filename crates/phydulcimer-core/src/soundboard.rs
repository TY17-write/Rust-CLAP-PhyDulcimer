...
//!
//! 板の方程式は解かない (PhyPiano P-002 と同じ判断)。板の伝達関数を
...
//! 駆動点が違えばモードへの結合が違う (PhyPiano P-040 の物理)。さらに
...
//! # モードの設計
//!
//! - 周波数: 対数等間隔 + ジッタ。**等間隔のままだと櫛になる** (PhyPiano
//!   P-039 — モードの帯域幅より間隔が広い帯域では、部分音が谷に落ちると
//!   20–30 dB 沈む)
//! - 減衰: 周波数依存 (低域ほど長い) × モードごとの散らし。**散らしの上限は
//!   クランプする** (P-039: クランプしないと鳴り止まないモードができる)
...
/// 1 ゾーンのモード数。
///
/// PhyPiano は 1 系統 160 → 480 本 (P-038 で増量)。こちらは 2 ゾーン合計で
/// 400 本。楽器が小さくモード密度は低いが、櫛を避けるには本数が要る。
...
impl Default for SoundboardParams {
    /// ダルシマーの薄い響板の暫定値。
    ///
    /// **実測ではない。** 実機の板厚 ~6 mm・面積 ~0.3 m² から、ピアノ
    /// (PhyPiano の校正値: 低域 0.30 s / 高域 0.05 s) より軽く短めに置いた。
    /// 掃引での置き直しは Phase 10。
    fn default() -> Self {
        Self {
            f_min_hz: 90.0,
...
mod tests {
    ...
    fn the_radiation_knob_shapes_the_low_end() {
        ...
        let level_100hz = |radiation_hz: f64| -> f64 {
            ...
            // 読みを支配する (PhyPiano の「Goertzel に窓を掛け忘れると低音の
            // 測定が壊れる」の再演をここでやった)。
            let n = x.len();
